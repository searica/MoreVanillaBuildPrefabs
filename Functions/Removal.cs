...
internal static class Removal
{
    ...
    internal static class PlayerPatch
    {
        ...
        internal static bool RemovePiecePrefix(Player __instance, ref bool __result)
        {
            ...
            {
                Transform cameraTrans = GameCamera.instance.transform;
                if (Physics.Raycast(cameraTrans.position, cameraTrans.forward, out RaycastHit hitInfo, 50f, PieceRemovalMask) &&
        ...
        {
            Log.LogInfo("Creating hit effects", Log.InfoLevel.Medium);

            EffectList.EffectData[] hitEffects = destructible.m_hitEffect?.m_effectPrefabs;
            if (hitEffects != null && hitEffects.Length != 0)
            {
                ...
                foreach (EffectList.EffectData effect in hitEffects)
                {
        ...
        private static bool RemoveMineRock5Piece(Piece piece)
        {
            ...
            {
                Log.LogInfo("Removing MineRock5 piece", Log.InfoLevel.Medium);
    ...
    internal static class PiecePatch
    {
        ...
        private static Piece.Requirement[] DropResources_m_resources_Delegate(Piece piece)
        {
            ...
            Log.LogInfo("Dropping resources for MVBP piece", Log.InfoLevel.Medium);

            // Set resources to defaults is piece is not placed by player (world-generated pieces)
            if (!piece.IsPlacedByPlayer() && 
                ZNetPrefabManager.TryGetVanillaPieceResources(piece.gameObject, out Piece.Requirement[] defaultResources) && 
        ...
        private static bool DestroyPrefix(Destructible __instance)
        {
            if (__instance && ZNetPrefabManager.IsPatchedByMVBP(__instance.gameObject))
            {
                ...
                if (piece && piece.IsPlacedByPlayer())
                {
                    ...
                    {
                        Log.LogInfo("Piece nview was destroyed during DropResources()", Log.InfoLevel.Medium);
                        return false;
                    }
                }
            }
    ...
    internal static class DropOnDestroyedPatch
    {
        ...
        private static bool OnDestroyedPrefix(DropOnDestroyed __instance)
        {
            if (__instance &&
                ...
                && piece.IsPlacedByPlayer())
            {
                Log.LogInfo("Disabling on destroyed drops for player-built object", Log.InfoLevel.Medium);
                return false;
            }
