...
## Deep North Notice
I have updated the mod to work with the Deep North update but I haven't had a chance to playthrough the new content yet so I have not enabled or patched any of the new pieces yet. Also, the config settings for the build piece category will need to be overhauled to allow users to control which tabs the new pieces show up but for now I have set up automated sorting for all pieces that I hope is reasonably intuitive for users.

Expect more updates in the near future to improve the configuration options and expand the default settings to enable more pieces by default. 
