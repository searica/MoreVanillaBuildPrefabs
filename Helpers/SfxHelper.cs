...
internal static class SfxHelper
{
    ...
    {
        // get effects
        EffectList placeEffects = piece?.m_placeEffect;
        if (placeEffects == null)
        {
            piece.m_placeEffect = new EffectList();
        }
        EffectList.EffectData[] effects = placeEffects?.m_effectPrefabs;
        ...
        CraftingStation craftingStation = piece?.m_craftingStation;
    ...
    {
        EffectList effects = FixRemovalSfx(piece);
        effects.Create(
    ...
    {
        CraftingStation craftingStation = wearNTear?.gameObject?.GetComponent<Piece>()?.m_craftingStation;
    ...
    {
        EffectList.EffectData[] effects = (effectList?.m_effectPrefabs) ?? (new EffectList.EffectData[0]);
        var effectsList = effects.ToList();
    ...
    {
        EffectList.EffectData[] effects = effectList?.m_effectPrefabs;
